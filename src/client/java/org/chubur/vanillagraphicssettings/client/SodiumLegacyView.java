...
public final class SodiumLegacyView {
        ...
        for(String name : new String[]{"net.minecraft.class_310", "net.minecraft.client.MinecraftClient", "net.minecraft.client.Minecraft"}) {
            try {
                ...
                for(Method method : type.getMethods()) {
                    if (Modifier.isStatic(method.getModifiers()) && method.getParameterCount() == 0 && method.getReturnType() == type && (method.getName().equals("method_1551") || method.getName().equals("getInstance"))) {
    ...
    private static Object findOptions(Object client) throws ReflectiveOperationException {
        for(Field field : client.getClass().getFields()) {
            ...
            if (name.equals("field_1690") || name.equals("options")) {
            ...
            try {
                ...
                for(Constructor<?> constructor : type.getConstructors()) {
                    ...
                    for(int index = 0; index < parameters.length && valid; ++index) {
                        if (parent != null && parameters[index].isInstance(parent)) {
                            arguments[index] = parent;
                        } else if (parameters[index].isInstance(client)) {
                            arguments[index] = client;
                        } else if (parameters[index].isInstance(options)) {
    ...
    private static void setScreen(Object client, Object screen) throws ReflectiveOperationException {
        for(Method method : client.getClass().getMethods()) {
            if ((method.getName().equals("method_1507") || method.getName().equals("setScreen")) && method.getParameterCount() == 1 && method.getParameterTypes()[0].isInstance(screen)) {
